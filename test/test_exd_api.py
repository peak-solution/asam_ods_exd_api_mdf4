...
class TestStringMethods(unittest.TestCase):
    ...
    def test_open(self):
        ...
        handle = service.Open(oed.Identifier(
            url=self._get_example_file_path('simple.mf4'),
            parameters = ""), None)
        try:
            pass
        finally:
    ...
    def test_get_values(self):
        ...
        try:
            ...
            self.assertSequenceEqual(values.channels[0].values.double_array.values, [0.0, 17905500.0, 30515300.0, 48317800.0])
            self.assertEqual(values.channels[1].values.data_type, ods.DataTypeEnum.DT_LONGLONG)
            self.assertSequenceEqual(values.channels[1].values.longlong_array.values, [1686082945335777300, 1686082945353682800, 1686082945366292600, 1686082945384095100])
