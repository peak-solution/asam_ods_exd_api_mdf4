    ...
    def test_datatype(self):
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            with MDF(version='4.10', file_comment="all_datatypes_test.mf4") as mdf4:
                ...
                sigs = []
                sigs.append(Signal(samples=np.array([1+2j, 3+4j], np.complex64),
                            timestamps=timestamps,
                            comment='complex64 data',
                            name='complex64_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([5+6j, 7+8j], np.complex128),
                            timestamps=timestamps,
                ...
                sigs = []
                sigs.append(Signal(samples=np.array([-2, 4], np.int8),
                            timestamps=timestamps,
                            comment='int8 data',
                            name='int8_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([2, 4], np.uint8),
                            timestamps=timestamps,
                            comment='uint8 data',
                            name='uint8_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([-2, 4], np.int16),
                            timestamps=timestamps,
                            comment='int16 data',
                            name='int16_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([2, 4], np.uint16),
                            timestamps=timestamps,
                            comment='uint16 data',
                            name='uint16_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([-2, 4], np.int32),
                            timestamps=timestamps,
                            comment='int32 data',
                            name='int32_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([2, 4], np.uint32),
                            timestamps=timestamps,
                            comment='uint32 data',
                            name='uint32_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([-2, 4], np.int64),
                            timestamps=timestamps,
                            comment='int64 data',
                            name='int64_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([2, 4], np.uint64),
                            timestamps=timestamps,
                            comment='uint64 data',
                            name='uint64_data',
                            unit='ns'))
                ...
                sigs = []
                sigs.append(Signal(samples=np.array([1.1, 1.2], np.float32),
                            timestamps=timestamps,
                            comment='float32 data',
                            name='float32_data',
                            unit='ns'))
                sigs.append(Signal(samples=np.array([2.1, 2.2], np.float64),
                            timestamps=timestamps,
                ...
                sigs = []
                sigs.append(Signal(samples=["abc", "def"],
                            timestamps=timestamps,
                            comment='string data',
                            name='string_data',
                            unit='ns'))
            ...
            try:
                ...
                values = service.GetValues(oed.ValuesRequest(handle=handle, group_id=0, start=0, limit=2, channel_ids=[0,1,2]), None)
                ...
                values = service.GetValues(oed.ValuesRequest(handle=handle, group_id=1, start=0, limit=2, channel_ids=[0,1,2,3,4,5,6,7,8]), None)
                ...
                values = service.GetValues(oed.ValuesRequest(handle=handle, group_id=2, start=0, limit=2, channel_ids=[0,1,2]), None)
                self.assertEqual(values.channels[1].values.data_type, ods.DataTypeEnum.DT_FLOAT)
                self.assertSequenceEqual(values.channels[1].values.float_array.values, [1.100000023841858, 1.2000000476837158])
