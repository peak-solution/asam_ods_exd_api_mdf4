...
class TestRoundtrip(unittest.TestCase):
    ...
    def test_assign_strings_falls_back_to_python_strings(self):
        target = ods.StringArray()
        section = np.array([b"alpha", b"beta"], dtype="S")
        channel = SimpleNamespace(data_type=99)

        ExternalDataFile._assign_strings(target, section, channel)

        self.assertSequenceEqual(list(target.values), ["alpha", "beta"])

    def test_assign_strings_converts_non_string_arrays(self):
        target = ods.StringArray()
        section = np.array([1, 2, 3], dtype=np.int64)
        channel = SimpleNamespace(data_type=99)

        ExternalDataFile._assign_strings(target, section, channel)

        self.assertSequenceEqual(list(target.values), ["1", "2", "3"])
    ...
    def _create_demo_file(self, name: str, create_array_group: bool) -> str:
        import numpy as np
        from asammdf import MDF, Signal
    ...
    def test_load_demo_file(self):
        """A start index past the last row must raise ValueError."""
        logger = logging.getLogger(__name__)
        path = self._create_demo_file("mdf4_demo_no_arrays.mf4", False)
        handle = self._open(path)
        try:
            struct = self._structure(handle)
            for group in struct.groups:
                group_id = group.id
                channel_ids = [ch.id for ch in group.channels]
                logger.info("Processing group %s with channels %s", group_id, channel_ids)
                self._values(handle, group_id, channel_ids, limit=1000)
        finally:
            self.service.Close(handle, None)
